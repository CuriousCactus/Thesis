...
In addition to its promising antibacterial activity, the cleavable conjugate \compound{cmpd:SHL4THCip} has an interesting growth curve (see \ref{fgr:growth}).
Whereas \textit{P. aeruginosa} treated with ciprofloxacin \compound{cmpd:Cip} continues to grow over the course of a 48 h assay, the growth curve for compound \compound{cmpd:SHL4THCip} remains flat. It seems that \compound{cmpd:SHL4THCip} kills/inhibits the growth all of the cells, whereas ciprofloxacin \compound{cmpd:Cip} either allows slow growth, or resistant mutants emerge and start to replicate.
